...
If `.claude-plugin/plugin.json` exists in cwd, refuse. You're inside the TStack plugin repo — these docs go in the *consumer* project, not here. Refusing also prevents you from clobbering the plugin's own `CLAUDE.md`.
...
Optionally update `.tstack/state.json` `stage` to `"architect"`.
