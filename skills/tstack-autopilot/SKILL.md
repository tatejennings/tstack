...
This is **not** a composition of `tstack-plan-milestone` + `tstack-build`. `tstack-plan-milestone` *is* its human-approval gate, which autonomy must skip. This skill is its own control flow that reuses their **artifact contracts**: the `docs/plans/{id}-plan.md` plan schema (`tstack-plan-milestone`), the quoted-"Done when" verification report (`tstack-build`), and the AGENTS.md / ROADMAP / M0 rules. Don't claim it re-runs those skills — it re-runs their contracts.
...
If a committed plan already exists at `docs/plans/{id}-plan.md`, use it (skip PLAN, still run RED-TEAM on it). If there's neither a committed plan nor a roadmap entry rich enough to plan from, refuse.
