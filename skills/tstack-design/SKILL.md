...
3. **Generate — choose a route** (how the artifact gets produced — default **A**):
   - **A · Claude Design (recommended; works at *both* fidelities).** Claude Design is Anthropic's browser design tool at **`claude.ai/design`** (Mac/Windows/Linux — no native app to install). It goes wireframe → high-fidelity, so **a wireframe pass still routes through it** — never defer Claude Design just because the visual layer isn't decided yet. *Its platform is unrelated to the product's target platform:* it designs native macOS/iOS apps, web apps, and CLI dashboards alike — never steer away from it because the product is "a Mac app" or "not web." Write a **structured, paste-ready** brief to `docs/3 - Design/claude-design-prompts.md`: **one kickoff prompt** (the design system + the **component inventory as the shared library**, ending by telling Claude Design to **ask clarifying questions and not design any screen yet — wait for the first screen prompt**), then **one self-contained, send-as-is prompt per screen** that reuses that library (the **first screen prompt is where designing begins**), then a **consistency-pass prompt** — each a single block the user pastes and sends, never loose fragments to assemble. The fidelity from step 1 sets what the kickoff carries:
...
> Design written to `docs/3 - Design/` — {n} screens, {n} components, {a DTCG token set if hi-fi}, and (Route A) a `claude-design-prompts.md` ready to paste into **Claude Design**.
>
> **Next — two paths:**
> - **Design first, then architect (recommended for Route A).** The prompts are a *brief, not finished mocks*. Paste them into `claude.ai/design`, generate the screens, then **bring the link or screenshots back** — I'll record them in `design.md`'s `## Design source` and fold any changes you made while designing into the spec. *Then* run **`tstack-architect`**, so the **frontend-stack ADR** and **ADR-3 (accessibility)** reflect the design you actually landed on, not the pre-generation draft.
> - **Continue to `tstack-architect` now.** Fine if the spec is already final, you used Route B/C (the artifact already exists), or you'll design later — link back whenever you do and I'll reconcile (the design-informed ADRs may want a revisit then).
>
> (Ran this mid-build for one screen? Just continue — nothing forces a chain step.)
