...
This repo *is* the **TStack** Claude Code plugin. It's not an application, it's not a documentation kit — it's a plugin you install into other projects so the TStack skills become available there: seven chained skills, one adoption on-ramp (`tstack-ingest`), three optional off-chain companions (`tstack-design`, `tstack-status`, `tstack-wrap`), plus a manual-only autonomous runner (`tstack-autopilot`). The deliverable is the contents of `.claude-plugin/` and `skills/`.
