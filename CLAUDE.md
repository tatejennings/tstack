...
This repo *is* the **TStack** Claude Code plugin. It's not an application, it's not a documentation kit — it's a plugin you install into other projects so the six TStack skills become available there. The deliverable is the contents of `.claude-plugin/` and `skills/`.
...
The chain: `discover → product → architect → roadmap → build`. Plus `specify` as the iteration loop, which hands off to `build`.
