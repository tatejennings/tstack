...
4. [x] **Loop-engineering positioning pass.** *(shipped in v0.3.2)* Editorial only. Reframe README + CLAUDE.md around the agentic loop: docs = durable external context, the "Done when" verify step = the engine, fresh session per milestone = the iteration. ("Loop engineering" is a soft community term; the substance — strong external verification + state-on-disk — is already TStack's core. Make it legible.) No behavior change.
