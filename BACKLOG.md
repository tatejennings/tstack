...
- [ ] **Verify the chain end-to-end in a scratch repo** (quality gate). Full chain (`discover` → `product` → `architect` → `roadmap` → `plan` → `build`) for a small real product. Confirm: the four foundational ADRs are produced, TESTING.md is generated, M0 is enforced, the AI branch fires when answered yes, and the verification report is properly quoted. Plus negative tests: invoke `tstack-roadmap` without M0 → expect refusal; invoke `tstack-specify-feature` → check the "considered, no change" line appears.
...
- [ ] **`tstack-ingest` v2 — adoption-aware `tstack-architect`** (extends the shipped on-ramp). v1 is product-grade only: foreign architecture docs are staged as `docs/_adopted/architecture-notes.md` and routed to `tstack-architect`, which asks its four ADR questions fresh (no adoption of technical docs — this is what structurally prevents ADR/M0 smuggling). (Design material now routes to `tstack-design`, which ratifies it into `docs/3 - Design/` — shipped in v0.7.0.) v2 would let architect *ratify* a rich foreign ARCHITECTURE.md from a draft (the same pattern product uses), while still forcing the four ADR questions. Reviewers flagged foreign arch docs rarely clear the bar and this reopens the ADR-enforcement surface — so only do it if real users show up with adopt-grade architecture docs. Decide at build time.
...
- [ ] **`tstack-techdebt`** (new skill) — periodic tech-debt audit. Reviews DECISIONS.md for ADRs that have aged poorly (e.g., "we picked Mongo for v1, now we have transactional pain"). Flags them and suggests a remediation milestone for ROADMAP.md. **Detector half of the pair**: the remediation it suggests is now *executed* by the shipped `tstack-refactor` (which writes the superseding ADR + behavior-preserving milestones).
- [ ] **`tstack-retire`** (new skill) — feature retirement. The *remove* corner of the evolve-an-existing-project family (`tstack-specify-feature` = add, `tstack-refactor` = restructure, this = remove): walks the doc set to remove a deprecated feature cleanly (PRODUCT.md scope, API.md endpoints, specs, ROADMAP entries, code paths).
